...
<?php
	...
	$errorName = __( 'Please enter your name.', 'xpro' );
	$errorEmail = __( 'Please enter a valid email address.', 'xpro' );
	$errorMessage = __( 'Please enter the message.', 'xpro' );
	...
	if ( isset( $_POST['is-submitted'] ) ) {
		...
		// Check the name
		if ( ! xpro_validate_length( $name, 2 ) ) {
			$isError             = true;
		...
		// Check the message
		if ( ! xpro_validate_length( $message, 2 ) ) {
			$isError                = true;
		...
		if ( ! $isError ) {
			...
			$emailSubject = sprintf( __( 'You have been contacted by %s', 'xpro' ), $name );
			$emailBody    = sprintf( __( 'You have been contacted by %1$s. Their message is:', 'xpro' ), $name ) . PHP_EOL . PHP_EOL;
			$emailBody    .= $message . PHP_EOL . PHP_EOL;
			$emailBody    .= sprintf( __( 'You can contact %1$s via email at %2$s', 'xpro' ), $name, $email );
				...
				<div class="entry-content">
					<?php if ( isset( $emailIsSent ) && $emailIsSent ) : ?>
						<div class="alert alert-success">
							<?php _e( 'Your message has been sucessfully sent, thank you!', 'xpro' ); ?>
						</div> <!-- end alert -->
					<?php else : ?>
					...
					<?php if ( isset( $isError ) && $isError ) : ?>
						<div class="alert-alert-danger">
							<?php _e( 'Sorry, it seems there was an error.', 'xpro' ); ?>
						</div> <!-- end alert -->
					<?php endif; ?>
					<?php endif; ?>
					...
					<form action="<?php the_permalink(); ?>" id="contact-form" method="POST" role="form">
						<div class="form-group <?php if ( isset( $errors['errorName'] ) ) echo "has-error"; ?>">
							<label for="contact-name" class="control-label"><span class="required">* </span><?php _e( 'Name:', 'xpro' ); ?></label>
						...
						<div class="form-group <?php if ( isset( $errors['errorEmail'] ) ) echo "has-error"; ?>">
							<label for="contact-email" class="control-label"><span class="required">* </span><?php _e( 'Email Address:', 'xpro' ); ?></label>
						...
						<div class="form-group <?php if ( isset( $errors['errorMessage'] ) ) echo "has-error"; ?>">
							<label for="contact-message" class="control-label"><span class="required">* </span><?php _e( 'Message:', 'xpro' ); ?></label>
						...
						<button type="submit" class="btn btn-default"><?php _e( 'Send Message', 'xpro' ); ?></button>
					</form>
